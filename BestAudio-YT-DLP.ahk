...
A_IconTip :=
    'YT-DLP AUDIO DOWNLOADER SCRIPT: AutoHotKey Script used by typing "~audio" into command prompt after copying the URL of the youtube video'
...
if (yt_dlp_path = "") {
    ...
    if !Input.Result || Trim(Input.Value) = ""  ; User canceled or entered empty string
    {
        MsgBox("No path was provided. The script will now close.")
        ExitApp
    }

    yt_dlp_path := Trim(Input.Value)
    ; Verify the path exists
    if !FileExist(yt_dlp_path) {
        MsgBox("The specified path does not exist. The script will now close.")
        ExitApp
    }
